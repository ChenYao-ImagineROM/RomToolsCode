...
class Extractor(object):
    ...
    def main(self, target, output_dir):
        ...
        if target_type == 'simg':
            print("正在把 %s 转换成 %s" % (os.path.basename(target), os.path.basename(target).replace(".img", ".raw.img")))
            self.__converSimgToImg(target)
            print("正在从 %s 中提取 %s" % (os.path.basename(target), os.path.basename(self.EXTRACT_DIR)))
